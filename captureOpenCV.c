...
int direction_flag=0;//<---------------------------------------------------donghun
void OverTaking(int direction_flag);
void UserInit(void);
...
static int Frame2Ipl(IplImage* img, IplImage* result)
{
   ...
   */
    ...
    for(j = 0; j < resHeight; j++)
    {
     ...
     if(left_obstacle>right_obstacle)  //<--------------------------------------------------------donghun
...
void UserInit(void)
{
	...
	PositionProportionPoint_Write(20); //Encoder -> P gain : default:10, range:10~50  /
	DesireSpeed_Write(50);                                                                       //speed
	EncoderCounter_Write(0);
	PositionControlOnOff_Write(UNCONTROL); // position control OFF // At first position controller must be OFF !!!
	SpeedControlOnOff_Write(CONTROL); // speed control OFF // speed controller must be also ON !!!
...
void *ControlThread(void *unused)
{
    ...
    while(1)
    {
        ...
        data = DistanceSensor(1); //channel num1
        printf("channel = %d, distance = 0x%04X(%d) \n", channel, data, data);
        if(data>600)  //if object detected, flag become 1.
        {
             printf("object detected!!");
             OverTaking(direction_flag);
...
void OverTaking(int direction_flag)
{
    ...
    if(direction_flag==1) //we need to turn right
	{
		...
		UserMove(1,1000,400);
		UserMove(1,1950,400);
		//UserMove(1,1460,3000);
		Winker_Write(ALL_OFF);
		SteeringServoControl_Write(1478); //initialize Servo Center Position  

		data = DistanceSensor(6); //channel num2           /
		while (data>100)
		{
			data = DistanceSensor(6); //channel num2
			//printf("channel = %d, distance = 0x%04X(%d) \n", channel, data, data);

...
void *ControlThread(void *unused)
{
	...
	DesireSpeed_Write(0);//Speed, default : 10, 
...
/*
		...
		*/
		if(interFlag == 1 && data > 700 && data < 800){
			SpeedControlOnOff_Write(CONTROL);   // speedControl ON
			//DesireSpeed_Write(40);
			SteeringServoControl_Write(1000);
			inPosition = EncoderCounter_Read();
			interFlag = 0;
		}
		if(interFlag == 0 && data > 1800){
			//DesireSpeed_Write(0);
		}else{
			//DesireSpeed_Write(40);
		}
		outPosition = EncoderCounter_Read();
		//1950
		/*
		if(outPosition - inPosition > 11800 && outPosition - inPosition < 44300){
			SteeringServoControl_Write(1950);
		}else if(outPosition - inPosition >= 44300){
